...
import { m } from './m'
import { projectSinksOn, makeDivVNode } from '../../utils/testing/checks'
  ...
  it("main cases - no children", (done) => {
    ...
    const mComponent = m({
      ...
      makeOwnSinks: (sources, settings) => ({
        DOM: $.map(user => h('div#container.two.classes', {}, [
          h('span', {style: {fontWeight: 'bold'}}, (user as any).key),
  ...
  it('main cases - children components and parent component - customized merge', (done) => {
    ...
    const mComponent = m({
      ...
      mergeSinks: (parentSinks, childrenSinks, settings) => ({
        ...
        auth$: parentSinks.auth$,
        user$: parentSinks.user$,
        childrenSinks$: $.mergeArray(projectSinksOn('DOM', childrenSinks)),
        settings$: $.of(settings),
      }),
      sinksContract: function checkMSinksContracts() {
        return true
